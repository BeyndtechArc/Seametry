...
fn a_donation_becomes_pending_then_vests() {
    ...
    world
        .send(
            ...
            sync_accounts(alloy, account),
        )
        .unwrap();
    let leg = world.alloy(alloy).legs[0];
    assert_eq!(
        (leg.ledger, leg.pending, leg.vest_start),
...
fn a_token_account_that_is_not_the_halls_is_refused() {
    ...
    let result = world.send(
        hall::instruction::Sync { leg_index: 0 },
        sync_accounts(alloy, stranger),
    );
    assert!(
        result.is_err(),
        "sync accepted an account that is not the Hall's"
    );
    assert!(
        logs(&result).contains("WrongHallAccount"),
        "{}",
        logs(&result)
    );
}
...
fn an_index_past_the_constituents_is_refused() {
    ...
    let result = world.send(
        hall::instruction::Sync { leg_index: 1 },
        sync_accounts(alloy, account),
    );
    assert!(result.is_err());
    assert!(
        logs(&result).contains("UnknownConstituent"),
        "{}",
        logs(&result)
    );
}
